...
1. Rearrange the barplot (Ranking)
2. Titles for all the plots and tables
3. Change the cover
4. Abstract, Introduction and Conclusion
...
For this assignment, we have tried to predict what variables impact the choice of voting for the Social Democratic Party of Sweden. The assignment is based on data from the European Social Survey from 2006. We modeled this using selected variables with Logit and Probit models. We found that most of our variables were significant and constant. Most of our variables were Likert variables, save for one.
The accuracy of our logit model was 61.1%, while the accuracy for our probit model was slightly higher, at 61.3%. Thus, we conclude that the probit model is better than the logit model, but the difference is very small.
...
2) Political interest could be a positve correlation, since the social democrats have been a major force in swedish politics for decades, and a high political interest could increase the chance to vote overall, instead of the individual staying at home due to having no interest in politics. 

3) A high family size is a socio-demographic factor which is often correlated with working class households - we expect larger households to have a higher chance to vote for SocDem for this reason. 

4) Finally, the typ of area that the individual is living in could also be a positive impact wether they vote SocDem, since a lot of industry is spread out over the smaller cities and countryside, where as cities might be expected to have a higher chance to vote for liberal parties.
...
- The parameter for the variable Area is not significant so it does not contribute any explanatory power.
...
Individual #18 is also a current member of a trade union. We increase the value of the var TradeUn to "2", which means he's a former member. According to our prediction, this makes him 9.67% more likely to vote SocDem. If we increase it further to "3", which means he's not a member, #18 is 16.6% more likely to vote SocDem. This might be because of the relationship between being a trade union member and having a high value in the variable TradeUn in inverted, the higher value instead being given to individuals that are not members. 
...
individual #18 has a family size of 3, a spouse and a child. We decrease the size of this family to just containing 1 member, to see how this affects the likelihood to vote for SocDem. This will allow us to see the change in probability when looking at an individual who is single compared to a member of a family. From our prediction, the change in probability is -4.67%. Similarly, individuals from families of a size of 2, i.e. a single parent or 2 spouses, gives a change in probability of -2.29%. If we instead increase from 3 to 4 members, we get an increase of 2.18%. This keeps increasing the more members above 3 we add - an individual from a family of 6 members has a probability increase of 6.24% for example. This might be because working-class households (which are probably more likely to vote SocDem) trend towards having larger families, and these households, in turn, form a large part of the SocDem voters.

To be generalized, when interpreting the change of possibility of voting for the democratic party when we change the value of the explanatory variables on individual observation, we always keep all the other explanatory variables as what it originally was. However, when we interpret the change of possibility for the observations altogether, we usually hold other variables at their means or medians for continuous variables and base level for categorical variables.

## 4) Likelihood Ratio test

### 1
We split the Political Intrest into dummy variables, now we have two models; the first one has Political intrest, Membership in the Trade Union, Family size and are, the second one has those same variables but Political Interest is split into dummy variables. We then apply the Likelihood ratio test.
...
We split the Family size into dummy variables, now we have two models; the first one has Political intrest, Membership in the Trade Union, Family size and are, the second one has those same variables but Family size is split into dummy variables. We then apply the Likelihood ratio test.
...
We split the Area dummy variables, now we have two models; the first one has Political intrest, Membership in the Trade Union, Family size and are, the second one has those same variables but Area is split into dummy variables. We then apply the Likelihood ratio test.

```{r}


m1 <- glm(as.factor(socialdemocrats) ~ PoInt+TradeUn+FamSize+as.factor(Area), data = dat_t, 
         family = binomial(link='logit'))
...
The recommended practice in this situation is to treat Political Intrest, Membership to the trade union and Family size as lickert variables, but attribute a different parameters to each dummy of the Area variable.
...
The goodness-of-fit measure taht we choose is the accuracy,. It is the ration of the correctly predicted observations to the total observations. Is calculated by:
Accuracy = $\frac{TP+TN}{TP+TN+FP+FN}$

where: TP = True positive; FP = False positive; TN = True negative; FN = False negative.

So the accuracy for our model is `r cm$overall[1]`
...
The accuracy for our model is `r cm$overall[1]`
