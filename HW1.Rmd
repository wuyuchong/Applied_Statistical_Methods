...
1. The names of two cut-off plots are wrong: should not be Chisquare Distribution - fixed
2. In the presentation part: The Likelihood Ratio-test part, constant should be constant scale - fixed
3. In the PDF, the section 8.5 Observed v. Predicted should be renamed for the reason that it is too casual. - fixed
4. Make a table for the likelyhood ratio test in the presentation. It should contain 3 columns: p_value, reject or not, constant scale or not. And contains 4 rows for 4 variables. - fixed
5. In the presentation, in the Comparison between observed and expected part, please add the table of confusion matrix from the report. - fixed
...
For this assignment, we tried to figure out what variables impact the choice of voting for the Social Democratic Party of Sweden. The thesis is based on a dataset from the European Social Survey from 2006. We selected variables and established Logit and Probit models. We found that most of our variables were significant and constant scale for the Likert variables. Most of our variables were Likert variables, except for one.
...
The accuracy for our model is `r cm1$overall[1]`.
...
A likelihood ratio test was also applied to our variables and only one of our variables was found to be non-constant - a variable that did not contribute any explanatory power to the logit model. We evaluated our logit model by measuring its accuracy, which was found to be 61.1%. Finally, we compared our logit model with a probit model using the same dataset. We found the accuracy of our probit model to be 61.3% - a difference of 0.02% percent, so rather small difference, but a difference nonetheless.

\newpage

# Appendix

table = cbind(preds, dat_t$socialdemocrats)
